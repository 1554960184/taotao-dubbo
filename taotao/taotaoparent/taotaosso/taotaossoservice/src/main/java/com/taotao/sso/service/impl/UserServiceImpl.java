...
public class UserServiceImpl implements UserService {
    ...
    public TaotaoResult register(TbUser user) {
        ...
        if (StringUtils.isNotBlank(user.getPhone())){
            TaotaoResult phone = checkData(user.getPhone(), 2);
            if (!(boolean)phone.getData()){
                return TaotaoResult.build(400,"手机号重复！");
            }
        }
        if (StringUtils.isNotBlank(user.getEmail())){
            TaotaoResult email = checkData(user.getEmail(), 3);
    ...
    public TaotaoResult login(String username, String password) {
        ...
        //把用户信息保存到redis 模拟session
        user.setPassword(null);
        jedisClient.set(USER_SESSION + ":"+token, JsonUtils.objectToJson(user));
        //设置key过期时间
        jedisClient.expire(USER_SESSION + ":"+token,SESSION_EXPIR);
